...
Framework-aware debugging for viact applications.

## Instructions

You are debugging a **viact** application — a full-stack Preact framework built on Vite. The user will describe a symptom (error, unexpected behavior, blank page, etc.). Investigate systematically using your knowledge of viact's architecture.
...
- Run `grep` for the route path across the manifest and check `matchAppRoute()` logic if needed.
